...
class SavesCardListFragment : Fragment() {
   ...
   fun initRecycleView(){
       binding.viewHeader.txtTitle.text=if (title.length==0)requireActivity().getString(R.string.saved_cards) else title
     //  val name=if (loginResponse.data.l_name.isEmpty())loginResponse.data.name else loginResponse.data.name +" "+loginResponse.data.l_name
       adaper = SavedCardAdapter(requireActivity(),dataList,"Schrood",isDelete,  { pos, type -> onAdapterClick(  pos,type) })
...
/*    private fun callCardListAPI( ) {
   ...
   private fun onAdapterClick( pos: Int,type: String) {
    ...
    fun dialogAddCard( ){
        val dialogBinding= DialogBottomAddCardBinding.inflate(LayoutInflater.from(requireActivity()), null, false)
        ...
        dialogBinding.btnAdd.setOnClickListener{
            ...
            if (TextUtils.isEmpty(dialogBinding.ediName.text.toString().trim())){
                ...
                utilsManager.showAlertMessageError(requireActivity(),requireActivity().getString(R.string.please_enter_name))
    ...
    private fun addCard(strName:String,card: CardMultilineWidget) {
         ...
         if (!card.validateAllFields()) {
           ...
           utilsManager.showAlertMessageError(requireActivity(), requireActivity().getString(R.string.invalid_card ))
        } else {
